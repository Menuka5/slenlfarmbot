...
define('CLOUDANT_URL', 'ramdesh.cloudant.com');
define('CLOUDANT_DB', 'farmbot');
define('CLOUDANT_USER', 'ramdesh');
define('CLOUDANT_PASS', '181mihiri');
...
class FarmBot {
    ...
    function __construct() {
        ...
        $this->_db = new Sag(CLOUDANT_URL, '80');
        $this->_db->login(CLOUDANT_USER, CLOUDANT_PASS);
        $this->_db->setDatabase(CLOUDANT_DB, false);
    }
...
');
    ...
    function send_response($input_raw) {
        ...
        $seq_id = $this->_db->get('_design/seq/_view/last-seq-id?limit=1&descending=true')->body->rows[0]->key;
        print_r($seq_id);
        ...
        $db = dbAccess::getInstance();
        //$response = $this->send_curl('https://api.telegram.org/bot112493740:AAHBuoGVyX2_T-qOzl8LgcH-xoFyYUjIsdg/getUpdates');
        /*$input_raw = '{
                         "update_id": 89023643,
                         "message": {
                           "message_id": 9370,
...
http://myteams.website/teams/8qIqaqwnZurf');
            $this->send_curl($this->build_response($chat_id, $reply));
            return;
        }
...
https://telegram.me/SlEnlFarmbot?startgroup=addmetogroup');
        ...
        if ($request_message == 'Cancel') {
            ...
            $this->send_curl('https://api.telegram.org/bot'. ACCESS_TOKEN. '/sendMessage?chat_id='
...
');         }
        ...
        if (in_array($request_message, $sequence_commands)) {
            ...
            $this->send_curl($this->build_response_keyboard($chat_id, $reply, $message_id, $keyboard));
            return;
        }
        ...
        if ($request_message == '/createfarm') {
            $time = $location = '';
            $farmer_name = '@' . $messageobj['message']['from']['username'];
            $reply .= $this->easter_eggs($farmer_name);
...
1. ' . $farmer_name);
        ...
        if (array_key_exists('reply_to_message', $messageobj['message'])) {
            ...
            if (strpos($reply_to_message, 'details') !== false) {
                ...
                $reply .= $this->build_farm_message($currentfarm['id']);
                $this->send_curl($this->build_response($chat_id, $reply));
...
');
            ...
            if (strpos($reply_to_message, 'location of') !== false) {
                ...
                $this->send_curl(build_location_response($chat_id,$locationobj));
...
'.substr($messageobj['message']['text'], 14));
            $this->send_curl($this->build_response(-27924249, $reply));
...
$farmbot->send_response(file_get_contents('php://input'));
